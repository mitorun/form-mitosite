...
- Форма может работать вообще без JS-скрипта, причём для этого ничего не нужно делать. Атрибут `action="/fls/phpmailer/mail.php"` направляет данные в PHP-скрипт и тот отправляет письмо. Только при этом в браузере открывается страница (белая-пустая) этого PHP-скрипта. <span style="color:red;">Идея для улучшения: доработать PHP-скрипт так, чтобы пользователя возвращало на ту же страницу с формой.</span>
