...
- Добавить атрибуты pattern к текстовым инпутам.
...
- Если отправить форму с непроставленной галочкой согласия на обработку данных, то страница проскроллится до этого чекбокса, но нет никакого видимого оповещения что тут проблема. Нужно придумать как при этом можно зрительно выделить этот инпут, может стили мешают показать встроенную HTML-валидацию?
...
- Когда к `<input type="tel">` подключается маска с помощью атрибута `data-tel-input` – то этот инпут теряет HTML-валидацию! В нём перестают работать атрибуты `minlength` и `maxlength` и появляться тултипы браузера с описанием ошибки.
